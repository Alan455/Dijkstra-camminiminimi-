...
negativi. L'algoritmo di Dijkstra richiede un nodo sorgente $s$, e
opera sfruttando una coda di priorità, che abbiamo già implementato
nei programmi [minheap.c](../solutions/minheap.c) e
[minheap.h](../solutions/minheap.h)).

Per i dettagli dell'algoritmo si faccia riferimento al materiale visto
in aula. Detta $d[v]$ è la distanza minima tra la sorgente e il nodo
$v$, $v \in \{0, \ldots, n-1\}$), l'algoritmo di Dijkstra inizializza
tutte le distanze a $+\infty$ per poi calcolare i valori corretti. Per
rappresentare il valore $+\infty$ si può usare il simbolo `HUGE_VAL`
definito in `<math.h>` che rappresenta l'infinito positivo[^1].
Questo simbolo è definito nello standard C90, per cui dovrebbe essere
presente in tutti i compilatori conformi.
...
puntatori ad archi `sp[]` sempre di lunghezza $n$. `p[v]` indica il
predecessore del nodo $v$ lungo il cammino minimo dalla sorgente a
$v$; se $v$ non ha predecessore (ad es., perché è il nodo sorgente o
perché non è raggiungibile dalla sorgente) si deve porre `p[v] = -1`
oppure `p[v] = NODE_UNDEF`. L'array `p[]` serve per poter ricostruire
i cammini minimi. `sp[v]` è un puntatore all'arco entrante nel nodo
$v$ lungo il cammino minimo dalla sorgente a $v$; se $v$ non è
raggiungibile dalla sorgente, oppure se $v$ coincide con la sorgente,
si pone `sp[v] = NULL`; si faccia riferimento alla Figura 1.
...
- il secondo parametro indica il nodo sorgente (se manca, si assume 0
  come sorgente);

- il terzo parametro indica un nodo destinazione. Si noti che il
  programma calcola sempre le distanze di *tutti* i nodi dalla
  sorgente, per cui il nodo destinazione influenza solo ciò che viene
  stampato ma non il costo asintotico dell'algoritmo. Se la
...
possono usare i file di input nella Tabella 1, che rappresentano
porzioni delle reti stradali di alcuni stati americani e della città
di Roma. Viste le dimensioni degli input, si suggerisce di disattivare
la stampa dei cammini minimi (commentare la chiamata alla funzione
`print_path()` dentro `print_dist()`).
...
I dati sono quelli forniti nel [9th DIMACS implementation
challenge--Shortest
Paths](https://www.diag.uniroma1.it/challenge9/download.shtml).
Prestare attenzione al fatto che i grafi non sono sempre (fortemente)
connessi, quindi non tutti i nodi sono raggiungibili dagli tutti
altri.
